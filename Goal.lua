		...
		if ZGV.GetTargetId()==self.npcid then
			if GossipFrame:IsShown() then
				...
				for qnum=1,GetNumGossipAvailableQuests() do
					...
					if frequency==Enum.QuestFrequency.Daily then noquests=nil break end  -- one daily breaks it
				end
...
GOALTYPES['havebuff'] = {
	...
	gettext = function(self) 
		if self.qspellid and not self.qspellname then self.qspellname = GetSpellInfo(310143) end
