...
local function _StartupThread()
	...
	local function startup_is_done (s)
		for i,startup in ipairs(self.startups) do  if startup.name==s then return startup.done end  end
	end
	...
	tinsert(self.startups,{name="guides_loaded",func=function() end,done=true}) -- oh geez. Insert a dummy startup entry to let startups use after="guides_loaded"
	...
	-- run "late" startups
	for cycles=1,20 do
		yield("Startup cycle two "..cycles)
		local done = run_startup_cycle()
		if done==0 then break end
	end
...
function ZGV:GetStickiesAt(stepnum,show_complete)
	...
	if step.stickies then
		for _,stickystep in ipairs(step.stickies) do
			if show_complete or not stickystep:IsComplete() then
...
function ZGV.IsLegionOn()
	return IsQuestFlaggedCompleted(44663) or ZGV:GetPlayerPreciseLevel()>=101
end
...
function ZGV.InPhase(phasename)
	...
	if phasename=="bfa" then
		if faction=="Horde" then 
			...
			return IsQuestFlaggedCompleted(50769) or (quest and quest.inlog)
		else
			local quest=ZGV.questsbyid[46728]
			return IsQuestFlaggedCompleted(46728) or (quest and quest.inlog)
		end
	...
	elseif phasename=="exilesreach" then
		return IsQuestFlaggedCompleted(58418)
