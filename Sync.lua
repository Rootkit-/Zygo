...
function Sync:CreatePacket_GuideStatus()
	...
	local packet =
		...
		.. ("##CH=%d,%s,%s"):format(UnitLevel("player"),select(2,UnitRace("player")),select(2,UnitClass("player")))  --,ZGV.Parser.classspecs[select(2,UnitClass("player"))][GetSpecialization() or 0] or "unspec"
...
function Sync:CreatePacket_SlaveRequest()
	return ("%s##GU=%s"):format(PACKETTYPE_SLAVEREQUEST,ZGV.CurrentGuide.title)
end
...
function Sync:OnChatReceived(prefix,msg,channel,sender,target,zoneChannelID,localID,name,instanceID)
	...
	self:Debug("|cffaaff00RCV |cffffffff[%s]: |r%s",sender,msg)
...
function Sync:Init()
	if not C_ChatInfo.IsAddonMessagePrefixRegistered(PREFIX) then
		...
		self.rcvframe = ZGV.ChainCall(CreateFrame("Frame"))
			...
			:RegisterEvent("PARTY_MEMBER_DISABLE")
			:RegisterEvent("PARTY_MEMBER_ENABLE")
			--:RegisterEvent("UNIT_FLAGS")
			--:RegisterEvent("UNIT_PHASE")
			:RegisterEvent("UNIT_CONNECTION")
			:SetScript("OnEvent", function(frame,event,...)
				if event=="CHAT_MSG_ADDON" then
					self:OnChatReceived(...)
				elseif event=="GROUP_ROSTER_UPDATE" or event=="PARTY_MEMBER_DISABLE" or event=="PARTY_MEMBER_ENABLE" then
...
function Sync:Deactivate()
	...
	if ZGV.CurrentGuide.type=="SHARED" then
		local tab, existed = ZGV.Tabs:GetSpecialTabFromPool("shared")
