...
function ZGV:Options_DefineOptionTables()
	...
	self.font_dialog_white:SetFont(ZGV.DIR .. "\\skins\\opensans.ttf", 12)
	...
	do
		...
		AddOption('showgreysellbutton',{ type = 'toggle',_default=true,width=200, set=function(i,v) Setter_Simple(i,v)  ZGV.Loot:SetUpGreySellButton()  ZGV.Loot.greysellbutton:SetShown(v) end, disabled=function() return not ZGV.db.profile.enable_vendor_tools end })  -- Show vendor button
	...
	do
		...
		AddOption('travelusehs',     { type = 'toggle', width = "double", disabled=function() return not self.db.profile.pathfinding end, _default=true,  
			set=function(i,v)
				self.db.profile.traveluseghs=v
				self.db.profile.travelusedhs=v
				Setter_Travel(i,v)
			end, })
		AddOption('traveluseitems',    { type = 'toggle', width = "double", disabled=function() return not self.db.profile.pathfinding end, _default=true,  
			set=function(i,v)
				self.db.profile.travelusewhistle=v
				Setter_Travel(i,v)
			end, })
		...
		AddOption('gear_selected_class',{
			...
			set = function(i,v) 
				...
				if v == select(3,UnitClass("player")) then
					...
					if ZGV.db.profile.gear_selected_spec==5 then ZGV.db.profile.gear_selected_spec=1 end
				else
	...
	do
			--[[ AddOption('n_nc_enabled',{ type = 'toggle', width = "double", set = function(i,v) Setter_Simple(i,v) ZGV.NotificationCenter:UpdateEnable() end, _default = true, })--]]

		AddOption('',{ type = 'description', name=L['opt_nc_popups_title'], font=ZGV.font_dialog_gray})
			AddOption('n_popup_enable',{ type = 'toggle', width = "full", _default = true })
			AddOption('n_popup_hideall',{ type = 'toggle', width = "full", _default = true, disabled = function() return not self.db.profile.n_popup_enable end,})

			AddOptionSep()
			--[[AddOption("notify_following",{type='description', font=ZGV.font_dialog})--]]
			AddOption('n_popup_guides',{ type = 'toggle', width = "full", _default = true, disabled = function() return not self.db.profile.n_popup_enable end,})
	...
	do
		...
		AddOption('wipe_settings',{ type = 'execute', func=function(i, value)
			if IsShiftKeyDown() then
				table.wipe(ZGV.db.profile)
				table.wipe(ZGV.db.char)
				table.wipe(ZGV.db.realm)
				ReloadUI()
			else
				--[[
				local cont = ZGV.GuideMenu.MainFrame.WideColumnOptions.AceContainer
				local chi = cont and cont.children
				for i,v in ipairs(chi) do if v.userdata[1]=="wipe_settings" then ScriptAnimationUtil.ShakeFrame(v.frame,{ {x=-5,y=0}, {x=5,y=0},{x=-5,y=0}, {x=5,y=0},{x=-5,y=0}, {x=5,y=0}}, 2.0, 0.04) return end end
				--]]
			end
		end})
