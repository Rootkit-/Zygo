...
function ItemScore:SetStatWeights(playerclass,playerspec,playerlevel)
	...
	if ItemScore.rules[self.playerclass][self.playerspec] then
		...
		print("Unknown spec",self.playerclass,self.playerspec)
	end
	...
	if self.playerlevel>=10 then
		...
		elseif self.playerclass=="DEMONHUNTER" then -- Demon Hunter
			self.playerDualWield = true
		elseif self.playerclass=="WARRIOR" and self.playerspec==2 and self.playerlevel>=14 then -- Fury Warriors
			self.playerDualWield = true
...
function ItemScore:GetHeirloomInfo(itemlink)
	-- in shadowland heirlooms no longer give exp bonus, so we are not protecting them anymore
	do return false, 60 end
