...
do
	...
	if Lib then
		...
		function Lib:SetupInitialQuickTravel(startnode)
			...
			for i,port in ipairs(Lib.data.portkeys) do repeat
				...
				local reject_reason = (function()
					...
					if port.mode=="hearth" and not Lib.cfg.use_hearth then  return "use_hearth off"  end  -- obviously
					if port.mode=="ghearth" and not Lib.cfg.use_ghearth then  return "use_ghearth off"  end  -- obviously
					if port.mode=="dhearth" and not Lib.cfg.use_dhearth then  return "use_dhearth off"  end  -- obviously
					if port.mode=="whistle" and not Lib.cfg.use_whistle then  return "use_whistle off"  end  -- obviously
					if port.item and not (port.mode and not_misc_item_modes[port.mode]) and not Lib.cfg.use_item_teleports then  return "use_item_teleports off"  end  -- don't use items other then hearthstones and whistle
					if port.is_astral and not Lib.cfg.use_astral_recall then  return "use_astral_recall off"  end  -- captain?
		...
		function Lib:ReportPath(endnode)
			...
			-- PREPARE NODES FOR DISPLAY. Assign titles based on situation.
				for n=1,#results do repeat
					...
					node.is_arrival = (a_b=="taxi_taxi" or a_b=="ship_ship" or a_b=="zeppelin_zeppelin" or a_b=="portal_portal" or a_b=="mole_misc" or text=="arrive" or a_b=="taxi_argusportal" or a_b=="taxi_ferry" or a_b=="taxi_eternalgateway")
				...
				and
					(
						( -- standing on the point
							...
							and getdist(sn,n1)<(tonumber(n1.radius) or (IsFlying("player") and STANDING_ON_NODE_RADIUS_FLIGHT or STANDING_ON_NODE_RADIUS))  -- standing on n1
		...
		function Lib:CheckMaxSpeeds()
			...
			local canfly_in_sl = canfly and false
		...
		function Lib:GetPlayerPosition()
			...
			if Lib.garrison_fix_test and (map==582 or map==590) and not x --[[ WoD garrisons --]] then return 0.5,0.5,map,nil,nil,"VIRTUAL" end
