...
function WorldQuests:Update()
	...
	if not WorldQuests.useCache then 
		...
		elseif continent==905 or continent==619 or WorldQuests.current_mapid==905 or WorldQuests.current_mapid==619 then
	...
	for ii,questItem in ipairs(display_quests) do 
		...
		if WQ_RowNum>0 and WQ_RowNum<ROW_COUNT+1 then 
			...
			if find_world_quest_step(questItem.questID,questItem.mapID) then
	...
	for i,quest in pairs(Quests) do
		if WorldQuests:IsValidQuest(quest) then
			if isChecked then
				...
				if find_world_quest_step(quest.questID,quest.mapID) then
...
function WorldQuests:SetFilters()
	...
	if not ZGV.db.profile.WQupdate82 then
		...
		ZGV.db.profile.WQupdate82 = true
	end
	if not ZGV.db.profile.WQupdate90 then
	...
	if not ZGV.db.profile.WQupdate921 then
		ZGV.db.profile.WQreputation[2465]=true
		ZGV.db.profile.WQupdate921 = true
	end
