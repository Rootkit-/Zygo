...
function markerproto:tostring()
	return ("#%d %.1f,%.1f"):format(self.m or 0,(self.x or 0)*100,(self.y or 0)*100)
end
...
function Pointer:SetupArrow()
	...
	if not self.ArrowFrame.ArrowIcon then
		self.ArrowFrame.ArrowIcon = CHAINC(CreateFrame("Button","ZygorGuidesViewerPointerArrow_Icon",self.ArrowFrameCtrl,"SecureActionButtonTemplate"))
...
function Pointer.ArrowFrame_OnClick(frame,button)
	...
	elseif button=="RightButton" then
		Pointer:ShowRouteMenu(Pointer.ArrowFrame)
	end
end
...
function Pointer:ShowRouteMenu(arrowframe)
	...
	UIDropDownFork_SetAnchor(arrowframe.routemenuframe, 0, 0, "TOP", arrowframe, "BOTTOM")
	...
	if simpleway then
		...
		if way.type=="manual" or not way.goal then
			tinsert(menu,{
					...
					func = function() ZGV.Pointer:RemoveWaypoint(way) ZGV:ShowWaypoints() end,
					notCheckable=true,
				})
		end
		...
		if route then
			...
			local dest_type = Pointer.DestinationWaypoint.type=="manual" and L['pointer_arrowmenu_route_destination_manual'] or L['pointer_arrowmenu_route_destination_guide']
	...
	EasyFork(menu,arrowframe.routemenuframe,nil,0,0,"MENU",10)
...
function Pointer:CycleWaypoint(delta,nocycle,step)
	...
	if CS then CS:CycleWaypoint(delta,nocycle) end
