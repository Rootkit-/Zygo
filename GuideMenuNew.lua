...
function GuideMenu:SearchQuest(questID)
	...
	local category
	local questtitle = ZGV.QuestDB.QuestNamesStored[questID] or ZGV.QuestDB.QuestNamesStored[questID] or C_QuestLog.GetTitleForQuestID(questID)
...
function GuideMenu:ShowRowMouseOver(row)
	...
	if row.group then return end

	if row.guide and row.guide.headerdata then 
