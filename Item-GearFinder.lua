...
local PAST_DUNGEONS_LIMIT = 3 -- how many levels can user be above previous expansion cap before we start ignoring its dungeon
...
function GearFinder:TrimDatabase() 
	...
	if not ZGV.ItemScore.Items then return end
	for i,instance in pairs(ZGV.ItemScore.Items) do
...
function GearFinder:IsValidDungeon(dungeon, instanceId)
	...
	if dungeon.expansionLevel>GetExpansionLevel() then return false, false, ident, 0, false, "no expansion "..dungeon.expansionLevel end
	...
	if dungeon.minLevel and dungeon.minLevel > (ItemScore.playerlevel+FUTURE_DUNGEONS_LIMIT) then return false, false, ident, 0, false, "need way higher level "..dungeon.minLevel end
	if maxScaleLevel < (ItemScore.playerlevel-PAST_DUNGEONS_LIMIT) then return false, false, ident, 0, false, "outleveled "..maxScaleLevel..":"..(ItemScore.playerlevel-PAST_DUNGEONS_LIMIT)  end
	...
	if dungeon.minLevel and dungeon.minLevel > ItemScore.playerlevel then return false, true, ident, maxScaleLevel, mythic, "need higher level "..dungeon.minLevel end
	...
	if dungeon.attunement_queston and not (IsQuestFlaggedCompleted(dungeon.attunement_queston) or ZGV.Parser.ConditionEnv.havequest(dungeon.attunement_queston)) then return false, true, ident, maxScaleLevel, mythic, "attunement needed" end
...
function GearFinder:ScoreDungeonItems()
	...
	for dungeon,dungeondata in pairs(ZGV.ItemScore.Items) do
		local valid, future, ident, maxscale, mythic, mythicplus, comment = GearFinder:IsValidDungeon(dungeondata.dungeon, dungeondata.instanceId)
			...
			for boss,bossdata in pairs(dungeondata) do
				if type(bossdata)=="table" then
					for _,itemlink in pairs(bossdata[player]) do
						...
						local qname if bossdata.quest and bossdata.quest[faction] then qname=C_QuestLog.GetTitleForQuestID(bossdata.quest[faction]) end -- prefetch quest name
		...
		if upgrade then
			...
			if upgrade.future then
				...
				if upgrade.quest then 
					button.itemencounter:SetText("Quest: "..(upgrade.questname or C_QuestLog.GetTitleForQuestID(upgrade.quest) or ""))
