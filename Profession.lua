...
function ZGV:CacheRecipes_Queued(profs)
	...
	for _,recipeid in pairs(all_recipes) do
		...
		--[[ optional, SHADOWLANDS POC
		local C = C_TradeSkillUI
		local spell = 309230
		local opts = C.GetOptionalReagentInfo(309230)

		local optionalReagents = {{count=1,itemID=itemID,slot=slot}}

		for slot,reagents in ipairs(opts) do
		    for index,itemID in ipairs(reagents.options) do
			optionalReagents[1].itemID=itemID
			optionalReagents[1].slot=slot
			local bonus = C.GetOptionalReagentBonusText(spell, 1, optionalReagents);
			print(slot,index,itemID,bonus)
		    end
		end
		--]]
