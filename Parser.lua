...
function ZGV:NeedsAnimatedPopup(variablesArray)
	...
	-- get server date, and use it to check if we need to apply any special features
		local a = C_DateAndTime.GetCurrentCalendarTime()
...
local ConditionEnv = {
	...
	_Setup = function(self)
		...
		for standing,num in pairs(ZGV.StandingNums) do registerLiteral(standing,num) end  -- created by Faction module
		...
		for i,fa in ipairs{"alliance","horde","neutral"} do  registerLiteral(fa,pfa==fa)  end
	...
	countcompletedq = function(...)
		...
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.completedq(id) then res = res + 1 end
		end
		return res
	end,
	...
	questactive = function(id,mapid) -- mapid is optional, for backwards compatibility
		...
		if mapid then
			for i,v in pairs(C_QuestLine.GetAvailableQuestLines(mapid)) do
				if v.questID==id then return true end -- it is part of current questline, good
			end
		end
		if C_TaskQuest.GetQuestTimeLeftSeconds(id) then return true end -- final fallback, does it have time remaining
		return false
	end,
	anyquestactive = function(...)
		local count = select("#", ...)
		for i = 1, count do
			local id = select(i, ...)
			if Parser.ConditionEnv.questactive(id) then return true end
		end
		return false
	end,
